...
# In this version I made a major bug fix. In the previous versions since I am
# ... shuffling a list and adding the correct answer to it meaning a total of 4
# ... possible answers. When the quiz is restarted the correct answer gets added
# ... to this list again each time meaning the correct answer shows up again and
# ... therefore the chance of the correct answer showing up increases.
# This makes it significantly easier to get 100 points in the quiz as all you
# ... have to do is restart it multiple times.
# I also found that the quiz would completely skipped through the user getting the
# ... answer correct and go to the next question. It also did this when they ran
# ... out of tries and instead of showing an output it would skip forward. This
# ... was because mainloop was not set in place so it could not update.
...
class Category:
    ...
    def createQuizWindow(self, quizMaster):
        ...
        self.categoryBoxFrame.place(x=(width * 18) / ORIGINAL_WIDTH, y=(height * 178) / ORIGINAL_HEIGHT,
                                    width=(width * 364) / ORIGINAL_WIDTH, height=(height * 64) / ORIGINAL_HEIGHT)
        ...
        self.categoryBox = ttk.Combobox(self.categoryBoxFrame, textvariable=self.chosenCategory, state="readonly",
        ...
        self.categoryBox.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                               width=(width * 356) / ORIGINAL_WIDTH, height=(height * 56) / ORIGINAL_HEIGHT)
        # Creating a frame for the restart button and placing the frame
        self.restartFrame = Frame(self.infoFrame, bg=BORDER_COLOR)
        self.restartFrame.place(x=(width * 18) / ORIGINAL_WIDTH, y=(height * 558) / ORIGINAL_HEIGHT,
        ...
        self.restartButton = Button(self.restartFrame, text="Restart Quiz", bg=BACKGROUND_COLOR1, fg=TEXT_COLOR,
                                    font=("Arial", int((width * 17) / ORIGINAL_WIDTH), "bold"),
                                    padx=(width * 10) / ORIGINAL_WIDTH, pady=(height * 10) / ORIGINAL_HEIGHT,
        ...
        self.restartButton.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
