...
# In this version I am adding the use of Profiles. These will eventually be saved
# ... in a text file so that the values can be saved when the user exits the quiz.
# As this quiz is very casual and not competitive it should not matter if they find
# ... a method to alter their scores. However, later I may choose to use a less
# ... common file type for the profiles such as JSON as it is not as easy to edit
# In this version all I am doing is creating the Profile Class and creating a window
# ... which will show up before the actual quiz. In this version the quiz has become
# ... unplayable but this is done to test the profiles update.
# In this version I have switched to a newer version of the Text File Converter
# ... module which has the ability to transfer the profile text file to a list
# Currently, users are unable to create new profiles but this will be implemented soon
# ... and must be done through the text file
# Fixed a bug which would let the user crash the quiz. If the confirm answer button was
# ... pressed when the user ran out of tries or got the answer correct the quiz would
# ... either output an error or crash the quiz. I fixed this by disabling the button
# ... before the time.sleep command which meant the button could not be pressed until
# ... the next one was created.
# Currently, the highscores will update but they are not written to the text file so
# ... progress is not saved. (This will be resolved in future updates)
...
class Category:
    ...
    def createQuizWindow(self, quizMaster, username):
        ...
        categoryInstance = Category.categoryNames.index(self.categoryTextVar.get())
...
class Questions:
    ...
    def check_answer(self):
        ...
        # otherwise if the text in the selected button is the same as the correct answer
            ...
            # There will be a delay so the user can see the response
            time.sleep(DELAY * 3)
...
def create_main_menu_window(masterWindow, loggedUser):
    ...
    def change_category_scores(master: object) -> object:
    ...
    categoryTitleLabel = Label(userScoresInnerFrame, text="Category", bg=BACKGROUND_COLOR2,
                               fg=BORDER_COLOR, font=("Arial", int((width * 18) / ORIGINAL_WIDTH), "bold",
                                                      "underline"),
                               padx=(width * 5) / ORIGINAL_WIDTH, pady=(height * 5) / ORIGINAL_HEIGHT,
                               anchor="center")
    categoryTitleLabel.place(x=(width * 8) / ORIGINAL_WIDTH, y=(height * 8) / ORIGINAL_HEIGHT,
