...
# In this version I will also be adding the ability for the user to add their
# ...  own categories. This means that they will be able to create their own
# ... quizzes.
# Since new categories are added a new menu is needed so that they can interact
# ... and all the questions and answers for their new category.
# I will also need to add this category to each profile since it will need a
# ... location to actually save all this new data.
...
class Category:
    ...
    def create_quiz_window(self, quizMaster, username):
    ...
    def set_high_score_data(self):
        ...
        if len(mainList) > 0:
            mainList.sort(key=operator.itemgetter(1), reverse=True)
        if len(mainList) < 10:
...
def create_main_menu_window(masterWindow, loggedUser):
    ...
    def add_category():
        """This function will be used to generate a menu which can be used to add a category to the quiz"""

        def callback_unfocus(event):
            """This function is used if the user clicks out of entry boxes"""
            if categoryEntryBoxVar.get().strip() == "":
                categoryEntryBoxVar.set("  Enter Category")
            if questionEntryBoxVar.get().strip() == "":
                questionEntryBoxVar.set("  Enter Question {}".format(len(questionListAddQuestion) + 1))
            if correctAnswerEntryBoxVar.get().strip() == "":
                correctAnswerEntryBoxVar.set("  Enter Correct Answer")
            if incorrectAnswerOneEntryBoxVar.get().strip() == "":
                incorrectAnswerOneEntryBoxVar.set("  Enter Incorrect Answer 1")
            if incorrectAnswerTwoEntryBoxVar.get().strip() == "":
                incorrectAnswerTwoEntryBoxVar.set("  Enter Incorrect Answer 2")
            if incorrectAnswerThreeEntryBoxVar.get().strip() == "":
                incorrectAnswerThreeEntryBoxVar.set("  Enter Incorrect Answer 3")

        def callback_category(event):
            """This function is used when the user clicks on the entry box and clears the text within"""
            categoryEntryBoxVar.set("  ")
            if questionEntryBoxVar.get().strip() == "":
                questionEntryBoxVar.set("  Enter Question {}".format(len(questionListAddQuestion) + 1))
            if correctAnswerEntryBoxVar.get().strip() == "":
                correctAnswerEntryBoxVar.set("  Enter Correct Answer")
        ...
        def callback_question(event):
            """This function is used when the user clicks on the entry box and clears the text within"""
            questionEntryBoxVar.set("  ")
            if categoryEntryBoxVar.get().strip() == "":
                categoryEntryBoxVar.set("  Enter Category")
            if correctAnswerEntryBoxVar.get().strip() == "":
                correctAnswerEntryBoxVar.set("  Enter Correct Answer")
        ...
        def callback_correct_answer(event):
            """This function is used when the user clicks on the entry box and clears the text within"""
            correctAnswerEntryBoxVar.set("  ")
            if categoryEntryBoxVar.get().strip() == "":
                categoryEntryBoxVar.set("  Enter Category")
            if questionEntryBoxVar.get().strip() == "":
                questionEntryBoxVar.set("  Enter Question {}".format(len(questionListAddQuestion) + 1))
        ...
        def callback_incorrect_answer_one(event):
            """This function is used when the user clicks on the entry box and clears the text within"""
            incorrectAnswerOneEntryBoxVar.set("  ")
            if categoryEntryBoxVar.get().strip() == "":
                categoryEntryBoxVar.set("  Enter Category")
            if questionEntryBoxVar.get().strip() == "":
                questionEntryBoxVar.set("  Enter Question {}".format(len(questionListAddQuestion) + 1))
        ...
        def callback_incorrect_answer_two(event):
            """This function is used when the user clicks on the entry box and clears the text within"""
            incorrectAnswerTwoEntryBoxVar.set("  ")
            if categoryEntryBoxVar.get().strip() == "":
                categoryEntryBoxVar.set("  Enter Category")
            if questionEntryBoxVar.get().strip() == "":
                questionEntryBoxVar.set("  Enter Question {}".format(len(questionListAddQuestion) + 1))
        ...
        def callback_incorrect_answer_three(event):
            """This function is used when the user clicks on the entry box and clears the text within"""
            incorrectAnswerThreeEntryBoxVar.set("  ")
            if categoryEntryBoxVar.get().strip() == "":
                categoryEntryBoxVar.set("  Enter Category")
            if questionEntryBoxVar.get().strip() == "":
                questionEntryBoxVar.set("  Enter Question {}".format(len(questionListAddQuestion) + 1))
        ...
        def validate_category():
            """This function is used to make sure the username the user enters meets the requirements"""
            categoryName = categoryEntryBoxVar.get().strip()
            if categoryName == "":
                tkinter.messagebox.showerror("Error", "Please don't leave it blank.")
            elif categoryName in Category.categoryNames:
                tkinter.messagebox.showerror("Error", "Sorry, that Category has already been created.")
            elif categoryName == "Enter a Category":
                tkinter.messagebox.showerror("Error", "Please enter a valid Category Name")
            elif len(questionListAddQuestion) == 0:
                tkinter.messagebox.showerror("Error", "Please ensure that there is atleast one question added.")
            else:
                add_category()
                tkinter.messagebox.showinfo("Information", "Category Created")

        def add_category():
            """This function will be used to properly format and add the category as an instance"""
            categoryData = []
            categoryData.append(categoryEntryBoxVar.get().strip())
            for question in questionListAddQuestion:
                categoryData.append(question)
            Categories.append(Category.from_list(categoryData))
            for profile in Profiles:
                profile.saveData.append([categoryEntryBoxVar.get().strip(), 0, 0, 0])
            ...
            writeTextFile("profiles.txt", "Profile", generate_new_profile_data())
            writeTextFile("categories.txt", "Category", generate_new_category_data())
            categoryCreateFrame.destroy()

        def add_question():
            """This function is used once the user has entered one question and will take them to the next"""
            global questionCount, questionListAddQuestion
            if questionEntryBoxVar.get().strip() == "" or correctAnswerEntryBoxVar.get().strip() == "" or \
                    incorrectAnswerOneEntryBoxVar.get().strip() == "" or \
                    incorrectAnswerTwoEntryBoxVar.get().strip() == "" or \
                    incorrectAnswerThreeEntryBoxVar.get().strip() == "":
                tkinter.messagebox.showerror("Error", "Please ensure that none of the fields are left blank.")
            elif questionEntryBoxVar.get().strip() == "Enter Question {}".format(
                    questionCount + 1) or correctAnswerEntryBoxVar.get().strip() == "Enter Correct Answer" or \
                    incorrectAnswerOneEntryBoxVar.get().strip() == "Enter Incorrect Answer 1" or \
                    incorrectAnswerTwoEntryBoxVar.get().strip() == "Enter Incorrect Answer 2" or \
                    incorrectAnswerThreeEntryBoxVar.get().strip() == "Enter Incorrect Answer 3":
                tkinter.messagebox.showerror("Error", "Please ensure that all the fields are valid.")
            else:
                questionListAddQuestion.append(
                    [questionEntryBoxVar.get().strip(), [correctAnswerEntryBoxVar.get().strip(),
                                                         [
                                                             incorrectAnswerOneEntryBoxVar.get().strip(),
                                                             incorrectAnswerTwoEntryBoxVar.get().strip(),
                                                             incorrectAnswerThreeEntryBoxVar.get().strip()]]])
                questionEntryBoxVar.set("  Enter Question {}".format(len(questionListAddQuestion) + 1))
                correctAnswerEntryBoxVar.set("  Enter Correct Answer")
                incorrectAnswerOneEntryBoxVar.set("  Enter Incorrect Answer 1")
        ...
        def go_back():
            """This function is used to exit out of the add category scene"""
            questionListAddQuestion.clear()
            categoryCreateFrame.destroy()

        questionListAddQuestion.clear()
        categoryCreateFrame = Frame(userInteractFrame, bg=TEXT_COLOR)
        categoryCreateFrame.bind("<Button-1>", callback_unfocus)
        categoryCreateFrame.place(x=(width * 0) / ORIGINAL_WIDTH, y=(height * 0) / ORIGINAL_HEIGHT,
                                  width=(width * 400) / ORIGINAL_WIDTH, height=(height * 640) / ORIGINAL_HEIGHT)
        inputCategoryFrame = Frame(categoryCreateFrame, bg=BORDER_COLOR)
        inputCategoryFrame.place(x=(width * 18) / ORIGINAL_WIDTH, y=(height * 18) / ORIGINAL_HEIGHT,
                                 width=(width * 364) / ORIGINAL_WIDTH, height=(height * 64) / ORIGINAL_HEIGHT)
        categoryEntryBoxVar = StringVar()
        categoryEntryBoxVar.set("  Enter Category")
        inputCategoryEntry = Entry(inputCategoryFrame, textvariable=categoryEntryBoxVar, bg=BACKGROUND_COLOR1,
                                   fg=TEXT_COLOR, font=("Arial", int((width * 17) / ORIGINAL_WIDTH), "bold"),
                                   justify='left')
        inputCategoryEntry.bind("<Button-1>", callback_category)
        inputCategoryEntry.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                                 width=(width * 356) / ORIGINAL_WIDTH, height=(height * 56) / ORIGINAL_HEIGHT)
        inputQuestionFrame = Frame(categoryCreateFrame, bg=BORDER_COLOR)
        inputQuestionFrame.place(x=(width * 18) / ORIGINAL_WIDTH, y=(height * 98) / ORIGINAL_HEIGHT,
                                 width=(width * 364) / ORIGINAL_WIDTH, height=(height * 64) / ORIGINAL_HEIGHT)
        questionEntryBoxVar = StringVar()
        questionEntryBoxVar.set("  Enter Question {}".format(len(questionListAddQuestion) + 1))
        inputQuestionEntry = Entry(inputQuestionFrame, textvariable=questionEntryBoxVar, bg=BACKGROUND_COLOR1,
                                   fg=TEXT_COLOR, font=("Arial", int((width * 17) / ORIGINAL_WIDTH), "bold"),
                                   justify='left')
        inputQuestionEntry.bind("<Button-1>", callback_question)
        inputQuestionEntry.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                                 width=(width * 356) / ORIGINAL_WIDTH, height=(height * 56) / ORIGINAL_HEIGHT)

        inputCorrectAnswerFrame = Frame(categoryCreateFrame, bg=BORDER_COLOR)
        inputCorrectAnswerFrame.place(x=(width * 18) / ORIGINAL_WIDTH, y=(height * 178) / ORIGINAL_HEIGHT,
                                      width=(width * 364) / ORIGINAL_WIDTH, height=(height * 59) / ORIGINAL_HEIGHT)
        correctAnswerEntryBoxVar = StringVar()
        correctAnswerEntryBoxVar.set("  Enter Correct Answer")
        inputCorrectAnswerEntry = Entry(inputCorrectAnswerFrame, textvariable=correctAnswerEntryBoxVar,
                                        bg=BACKGROUND_COLOR1,
                                        fg=TEXT_COLOR, font=("Arial", int((width * 17) / ORIGINAL_WIDTH), "bold"),
                                        justify='left')
        inputCorrectAnswerEntry.bind("<Button-1>", callback_correct_answer)
        inputCorrectAnswerEntry.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                                      width=(width * 356) / ORIGINAL_WIDTH, height=(height * 51) / ORIGINAL_HEIGHT)

        inputIncorrectAnswerOneFrame = Frame(categoryCreateFrame, bg=BORDER_COLOR)
        inputIncorrectAnswerOneFrame.place(x=(width * 18) / ORIGINAL_WIDTH, y=(height * 253) / ORIGINAL_HEIGHT,
                                           width=(width * 364) / ORIGINAL_WIDTH,
                                           height=(height * 59) / ORIGINAL_HEIGHT)
        incorrectAnswerOneEntryBoxVar = StringVar()
        incorrectAnswerOneEntryBoxVar.set("  Enter Incorrect Answer 1")
        inputIncorrectAnswerOneEntry = Entry(inputIncorrectAnswerOneFrame,
                                             textvariable=incorrectAnswerOneEntryBoxVar,
                                             bg=BACKGROUND_COLOR1,
                                             fg=TEXT_COLOR,
                                             font=("Arial", int((width * 17) / ORIGINAL_WIDTH), "bold"),
                                             justify='left')
        inputIncorrectAnswerOneEntry.bind("<Button-1>", callback_incorrect_answer_one)
        inputIncorrectAnswerOneEntry.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                                           width=(width * 356) / ORIGINAL_WIDTH,
                                           height=(height * 51) / ORIGINAL_HEIGHT)

        inputIncorrectAnswerTwoFrame = Frame(categoryCreateFrame, bg=BORDER_COLOR)
        inputIncorrectAnswerTwoFrame.place(x=(width * 18) / ORIGINAL_WIDTH, y=(height * 328) / ORIGINAL_HEIGHT,
                                           width=(width * 364) / ORIGINAL_WIDTH,
                                           height=(height * 59) / ORIGINAL_HEIGHT)
        incorrectAnswerTwoEntryBoxVar = StringVar()
        incorrectAnswerTwoEntryBoxVar.set("  Enter Incorrect Answer 2")
        inputIncorrectAnswerTwoEntry = Entry(inputIncorrectAnswerTwoFrame,
                                             textvariable=incorrectAnswerTwoEntryBoxVar,
                                             bg=BACKGROUND_COLOR1,
                                             fg=TEXT_COLOR,
                                             font=("Arial", int((width * 17) / ORIGINAL_WIDTH), "bold"),
                                             justify='left')
        inputIncorrectAnswerTwoEntry.bind("<Button-1>", callback_incorrect_answer_two)
        inputIncorrectAnswerTwoEntry.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                                           width=(width * 356) / ORIGINAL_WIDTH,
                                           height=(height * 51) / ORIGINAL_HEIGHT)

        inputIncorrectAnswerThreeFrame = Frame(categoryCreateFrame, bg=BORDER_COLOR)
        inputIncorrectAnswerThreeFrame.place(x=(width * 18) / ORIGINAL_WIDTH, y=(height * 403) / ORIGINAL_HEIGHT,
                                             width=(width * 364) / ORIGINAL_WIDTH,
                                             height=(height * 59) / ORIGINAL_HEIGHT)
        incorrectAnswerThreeEntryBoxVar = StringVar()
        incorrectAnswerThreeEntryBoxVar.set("  Enter Incorrect Answer 3")
        inputIncorrectAnswerThreeEntry = Entry(inputIncorrectAnswerThreeFrame,
                                               textvariable=incorrectAnswerThreeEntryBoxVar, bg=BACKGROUND_COLOR1,
                                               fg=TEXT_COLOR,
                                               font=("Arial", int((width * 17) / ORIGINAL_WIDTH), "bold"),
                                               justify='left')
        inputIncorrectAnswerThreeEntry.bind("<Button-1>", callback_incorrect_answer_three)
        inputIncorrectAnswerThreeEntry.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                                             width=(width * 356) / ORIGINAL_WIDTH,
                                             height=(height * 51) / ORIGINAL_HEIGHT)
        addQuestionFrame = Frame(categoryCreateFrame, bg=BORDER_COLOR)
        addQuestionFrame.place(x=(width * 208) / ORIGINAL_WIDTH, y=(height * 478) / ORIGINAL_HEIGHT,
                               width=(width * 174) / ORIGINAL_WIDTH, height=(height * 64) / ORIGINAL_HEIGHT)
        addQuestionButton = Button(addQuestionFrame, text="Add Question", bg=BACKGROUND_COLOR1, fg=TEXT_COLOR,
                                   font=("Arial", int((width * 17) / ORIGINAL_WIDTH), "bold"),
                                   padx=(width * 10) / ORIGINAL_WIDTH, pady=(height * 10) / ORIGINAL_HEIGHT,
                                   relief="flat", activebackground=BACKGROUND_COLOR2, activeforeground=TEXT_COLOR,
                                   command=add_question)
        addQuestionButton.bind("<Button-1>", callback_unfocus)
        addQuestionButton.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                                width=(width * 166) / ORIGINAL_WIDTH, height=(height * 56) / ORIGINAL_HEIGHT)
        returnFrame = Frame(categoryCreateFrame, bg=BORDER_COLOR)
        returnFrame.place(x=(width * 18) / ORIGINAL_WIDTH, y=(height * 558) / ORIGINAL_HEIGHT,
                          width=(width * 174) / ORIGINAL_WIDTH, height=(height * 64) / ORIGINAL_HEIGHT)
        returnButton = Button(returnFrame, text="Go Back", bg=BACKGROUND_COLOR1, fg=TEXT_COLOR,
                              font=("Arial", int((width * 17) / ORIGINAL_WIDTH), "bold"),
        ...
        returnButton.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                           width=(width * 166) / ORIGINAL_WIDTH, height=(height * 56) / ORIGINAL_HEIGHT)
        categoryConfirmFrame = Frame(categoryCreateFrame, bg=BORDER_COLOR)
        categoryConfirmFrame.place(x=(width * 208) / ORIGINAL_WIDTH, y=(height * 558) / ORIGINAL_HEIGHT,
                                   width=(width * 174) / ORIGINAL_WIDTH, height=(height * 64) / ORIGINAL_HEIGHT)
        categoryConfirmButton = Button(categoryConfirmFrame, text="Confirm", bg=BACKGROUND_COLOR1, fg=TEXT_COLOR,
                                       font=("Arial", int((width * 17) / ORIGINAL_WIDTH), "bold"),
                                       padx=(width * 10) / ORIGINAL_WIDTH, pady=(height * 10) / ORIGINAL_HEIGHT,
                                       relief="flat", activebackground=BACKGROUND_COLOR2, activeforeground=TEXT_COLOR,
                                       command=validate_category)
        categoryConfirmButton.bind("<Button-1>", callback_unfocus)
        categoryConfirmButton.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                                    width=(width * 166) / ORIGINAL_WIDTH, height=(height * 56) / ORIGINAL_HEIGHT)
    ...
    def play_quiz():
        if profileVar.get() == "Select Profile" or chosenCategory.get() == "Select a Category":
            ...
            menuWindow.quit()
            Categories[Category.categoryNames.index(chosenCategory.get())].create_quiz_window(menuWindow,
                                                                                            profileVar.get())
    ...
    scriptLabelVar.set("1. If you are a returning user please select your profile from the menu on the right\n2. If "
                       ...
                       "Select which Category you would like to be tested on. You can create your own quiz using the "
                       "corresponding “+”.\n4. Then click “Play Quiz”. Which will begin the quiz.\n5. Your highest "
                       "scores will be saved for each quiz and can also be seen on the right.\nGood Luck!")
    ...
    addProfileFrame = Frame(userInteractFrame, bg=BORDER_COLOR)
    addProfileFrame.place(x=(width * 318) / ORIGINAL_WIDTH, y=(height * 18) / ORIGINAL_HEIGHT,
                          width=(width * 64) / ORIGINAL_WIDTH, height=(height * 64) / ORIGINAL_HEIGHT)
    # Creating a button within the addProfileFrame used to add a profile
    addProfileButton = Button(addProfileFrame, text="+", bg=BACKGROUND_COLOR1, fg=TEXT_COLOR,
                              font=("Arial", int((width * 35) / ORIGINAL_WIDTH), "bold"),
                              padx=(width * 10) / ORIGINAL_WIDTH, pady=(height * 10) / ORIGINAL_HEIGHT,
                              relief="flat", activebackground=BACKGROUND_COLOR2, activeforeground=TEXT_COLOR,
                              command=add_profile)
    # Placing the button in the correct position
    addProfileButton.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
    ...
    categoryBox.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                      width=(width * 276) / ORIGINAL_WIDTH, height=(height * 56) / ORIGINAL_HEIGHT)
    # Creating a frame for the add profile button and placing it
    addCategoryFrame = Frame(userInteractFrame, bg=BORDER_COLOR)
    addCategoryFrame.place(x=(width * 318) / ORIGINAL_WIDTH, y=(height * 98) / ORIGINAL_HEIGHT,
                           width=(width * 64) / ORIGINAL_WIDTH, height=(height * 64) / ORIGINAL_HEIGHT)
    # Creating a button within the addCategoryFrame used to add a profile
    addCategoryButton = Button(addCategoryFrame, text="+", bg=BACKGROUND_COLOR1, fg=TEXT_COLOR,
                               font=("Arial", int((width * 35) / ORIGINAL_WIDTH), "bold"),
                               padx=(width * 10) / ORIGINAL_WIDTH, pady=(height * 10) / ORIGINAL_HEIGHT,
                               relief="flat", activebackground=BACKGROUND_COLOR2, activeforeground=TEXT_COLOR,
                               command=add_category)
    # Placing the button in the correct position
    addCategoryButton.place(x=(width * 4) / ORIGINAL_WIDTH, y=(height * 4) / ORIGINAL_HEIGHT,
                            width=(width * 56) / ORIGINAL_WIDTH, height=(height * 56) / ORIGINAL_HEIGHT)
...
def generate_new_category_data():
    """This function will generate a list correctly formatted so it can be written to the categories file"""
    mainList = []
    for category in Categories:
        subList = []
        subList.append(category.categoryName)
        for question in category.questionList:
